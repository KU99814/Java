...
public class BinaryTree<E extends Comparable<E>> extends AbstractTree<E>
{
 ...
 public void breadthFirstTraversal()
  {
   Queue<TreeNode<E>> q = new LinkedList<TreeNode<E>>();//儲存node佇列
   q.offer(root);//將root存入
   TreeNode<E> current = null;//目前node

   //將queue中node取出顯示 並將左右子node放入queue
   while((current = q.poll()) != null)
    {
     System.out.println(current.element);//顯示
     //判斷是否有子node並放入queue
 ...
 public int depth()
  {
   Queue<TreeNode<E>> q = new LinkedList<TreeNode<E>>();//尋訪暫存
   Stack<TreeNode<E>> st = new Stack<TreeNode<E>>();//儲存leaf
   q.offer(root);//放入root
   ...
   while((current = q.poll()) != null)
    {
     ...
     if(current.left == null && current.right == null) 
      st.push(current);//leaf放入stack
    }

   java.util.ArrayList paths;//往node路徑

   while(!st.empty())
    {
     current = st.pop();//取出stack中node
     paths = path(current.element);//找出從root到node的路徑
     
     //判斷路徑長度是否最大 若是即為depth
