 ...
 protected void inorder(TreeNode<E> root)
  {
   ...
   boolean findLeft = true;//判斷是否有左node
   ...
   while(!st.empty())
    {
     ...
     while(findLeft)
      {
       ...
       if(current.left !=null)//若有左node 放入stack
        {
         st.push(current.left);
        }
       else if(current.right !=null)//若只有右子樹 先顯示目前node後放入右node
        {
     ...
     if(st.empty())//stack為空跳出
      break;
     
     current = (TreeNode<E>)(st.pop());//排出目前node
     System.out.print(current.element+" ");//顯示

           
     if(current.right !=null)//若有右子樹 放入stack繼續尋訪左子樹
      {
       findLeft = true;
       st.push(current.right);
      }
