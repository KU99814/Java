...
public class BinaryTree<E extends Comparable<E>> extends AbstractTree<E>
{
 ...
 //找出指定深度node的個數
 public int countDepth(int key)
  {
   ArrayList<TreeNode<E>> q = new ArrayList<TreeNode<E>>();//存放node的list
   int index = 0;//目前index
   ...
   java.util.ArrayList paths;//路徑 計算深度用
   ...
   while(index < q.size())
    {
     ...
     //找出路徑
     paths = path(current.element);

     //路徑長度=深度 
     //因為是廣度尋訪 同樣深度會一起出現
     if((paths.size()-1)==key)
      count++;
     else if((paths.size()-1)>key)//若遇到大於 跳出
      break;
