...
public abstract class AbstractGraph implements Graph
{
 ...
 public boolean isCyclic()
  {
   int[] parent = new int[vertices.length];//vertex parent陣列

   //初始全設為1
   for(int i=0;i<parent.length;i++)
    parent[i] = -1;

   java.util.LinkedList<Integer> queue = new java.util.LinkedList<Integer>();//放置vertex的queue
   boolean[] isVisited = new boolean[vertices.length];//判斷是否已尋訪
   queue.offer(0);//放入初始v

   isVisited[0] = true;//已尋訪設為true
   
   //作bfs尋訪
   while(!queue.isEmpty())
    {
     int u = queue.poll();//取出element
     
     //尋訪鄰居
     for(int w:neighbors[u])
      {
       //若已尋訪且並非parent 代表可組成一cycle
       //因為bfs是一層層尋訪 在最初必定相連
       if(isVisited[w] && w!=parent[u])
        return true;//回傳true
   ...
   return false;//若沒找到 回傳false
  }

 //尋找點到點的路徑
 public List getPath(int u,int v)
  {
   Tree tree = bfs(u);//創建bfs樹 以u為root

   Iterator iterator = tree.pathIterator(v);//以v為目標作尋訪

   List<Integer> list = new ArrayList<Integer>();//path list

   //將iterator element 放入list
   ...
   //如果list最後是v 代表有路徑 否則回傳空
   if(list.get(list.size()-1) == v)
    return list;
   else
    return null;
  }
