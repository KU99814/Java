...
public abstract class AbstractGraph implements Graph
{
 ...
 public List isBipartite()
  {
   List<Integer> searchOrders = new ArrayList<Integer>();//尋訪路徑
   
   int[] ncolor = new int[vertices.length];//vertex著色 有1和-1兩種

   int color = 1;//現在的顏色

   java.util.LinkedList<Integer> queue = new java.util.LinkedList<Integer>();//暫存queue
   ...
   while(!queue.isEmpty())
    {
     ...
     searchOrders.add(u);//加入尋訪路徑
     
     color = ncolor[u]*-1;//顏色改變 每一迴圈是新的一層
     ...
     for(int w:neighbors[u])
      if(!isVisited[w])
       {
        queue.offer(w);
        ncolor[w] = color;//放入現在顏色
        isVisited[w] = true;
       }
    }

   //按順序檢查每一vertex的鄰居
   //因為二分圖中 相同獨立集合中的vertex彼此必不相連
   //若相鄰vertex的color相同 代表不是二分圖
   for(int i=0;i<vertices.length;i++)
    {
     //檢查鄰居的color
     for(int j:neighbors[i])
      {
       if(ncolor[i] == ncolor[j])
        {
         return null;//有相同 不是二分圖 傳null
        }
      }
    }
   ...
   List<List<Integer>> reList = new ArrayList<List<Integer>>();//回傳的list
