 ...
 public int getIndex(Object vertex)
  {
   ...
   for(int i=0;i<getSize();i++)
    {
     if(vertex.equals(vertices[i]))//尋找到相同vertex時回傳index
      return i;
    }
 ...
 public Tree dfs(int v)
  {
   ...
   Stack<Integer> stack = new Stack<Integer>();//途中儲存stack
   
   stack.push(v);//將指定index放入stack

   //利用stack先進後出特性 讓最後放入的vertex始終先處理
   //達成深度搜尋之目的
   while(!stack.isEmpty())
    {
     int u = (int)(stack.pop());//取出index
     ...
     if(!isVisited[u])
      {
       //若否 加入走訪list
       searchOrders.add(u);
       isVisited[u] = true;//走訪的index設為true
      }

     //將相連vertex中 未走訪過之vertex index放入stack
     for(int i:neighbors[u])
      if(!isVisited[i])
       {
        //這裡先放目前vertex 因為子vertex可能彼此相連
        //導致將兄弟vertex設為parent
        //因此深度走訪完要回到最近共同祖先vertex 重設parent
        parent[i] = u;
        stack.push(u);
        stack.push(i);
        break;
   ...
   return new Tree(v,parent,searchOrders);//創建spinner tree並回傳
  }
 ...
 public List<Integer> getHamiltonianPath(int v)
  {
   ...
   //根據分支度重新排序相鄰關係順序
   for(int i=0;i<getSize();i++)
    reorderNeighborsBasedOnDegree(neighbors[i]);//call method
 ...
 public class Tree
  {
   ...
   public void printPath(int v)
    {
     Iterator iterator = pathIterator(v);//iterator
