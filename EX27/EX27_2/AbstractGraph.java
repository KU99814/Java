...
public abstract class AbstractGraph implements Graph
{
 ...
 //找出graph中相連部分
 public List getConnectComponent()
  {
   boolean[] isVisited = new boolean[vertices.length];//判斷vertex是否有拜訪過
   List<Tree> list = new ArrayList<Tree>();//相連部分list
   ...
   for(int i=0;i<vertices.length;i++)
    {
     ...
     if(!isVisited[i])
      {
       Tree tree = dfs(i);//以目前vertex做root來做dfs 因為dfs只會尋訪相連的vertex
       List<Integer> searchOrders = tree.getSearchOrders();
       //將tree中的vertex設為已尋訪
       for(int j=0;j<searchOrders.size();j++)
        {
         int n = (int)(searchOrders.get(j));
         isVisited[n] = true;
        }
       list.add(tree);//將tree放入list
      }
    }
